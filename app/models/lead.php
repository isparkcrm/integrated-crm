...
public function getleadtotal1($accountmanager)
{
	...
	$this->db->query('SELECT sum(ordervalue) as total from lead where assignee=:user and leadstatus="Lead" and status=:status1  OR assignee=:user and leadstatus="Lead" and status=:status2');
	$this->db->bind(':user', $accountmanager);
	$this->db->bind(':status1', $status1);
	  $this->db->bind(':status2', $status2);
	$row = $this->db->single();
...
public function getcoldtotal1($accountmanager)
{
	...
	$this->db->query('SELECT sum(ordervalue) as total from lead where assignee=:user and leadstatus="Cold" and status=:status1  OR assignee=:user and leadstatus="Cold" and status=:status2');
	$this->db->bind(':user', $accountmanager);
	$this->db->bind(':status1', $status1);
	  $this->db->bind(':status2', $status2);
	$row = $this->db->single();
